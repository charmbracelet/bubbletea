...
// Sequence mappings.
//
// Note: if you modify this map, do not forget to re-run `go generate`.
...
// The following generator needs golex installed;
// install with:
//     go install modernc.org/golex@latest

//go:generate go run key/gen.go
...
func detectOneMsg(b []byte) (w int, msg Msg) {
	...
	// because of a limitation in golex.
	if i < len(b) && b[i] == 0 {
