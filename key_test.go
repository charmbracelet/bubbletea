...
func TestReadLongInput(t *testing.T) {
	...
	if len(k.Runes) != 1000 {
		t.Errorf("expected 1000 runes, got %d", len(k.Runes))
	}
	if !reflect.DeepEqual(k.Runes, []rune(input)) {
...
// extSequences is used by the map-based algorithm below. It ccontains
// the sequences plus their alternatives with an escape character
// prefixed, plus the control chars, plus the space, i.e. all the
// things detected otherwise by the lex-based detector.
...
// detectSequenceMap is an alternative to the lex-based sequence
// detector, using a longest prefix match over the input sequence and
// a hash map.
func detectSequenceMap(input []byte) (hasSeq bool, width int, msg Msg) {
	seqs := extSequences
...
func TestDetectRandomSequencesMap(t *testing.T) {
	runTestDetectSequence(t, detectSequenceMap)
}

// BenchmarkDetectSequenceLex benchmarks the lex-generated sequence
// detector.
func BenchmarkDetectSequenceLex(b *testing.B) {
...
func BenchmarkDetectSequenceMap(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		for j, w := 0, 0; j < len(td.data); j += w {
			_, w, _ = detectSequenceMap(td.data[j:])
		}
	}
}
