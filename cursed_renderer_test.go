...
func TestCursedRenderer_restoresKittyKeyboardStack(t *testing.T) {
	...
	// the flush after the renderer was restarted, and when leaving the
	// alt screen back to the main screen. Entering the alt screen pushes
	// into the alt screen's own stack, which uses the default flags here.
	// start() itself does not write to [out].
	...
	// Every pop must come after the push of the screen it belongs to, and
	// the final pop must come last.
